...
| Blocker | Summary | Fix |
|---------|---------|-----|
| C1 | Planner produces `ExecutionPlan`, Runtime consumes `ExecutionPlanInput` — no adapter | Created `app/orion/plan_adapter.py` |
| C2 | Workflow Runtime registered in kernel but never called from orchestrator | Modified `OrionOrchestrator` to route plans through `RuntimeSchedulerBridge` |
...
```
app/orion/plan_adapter.py                        (new, 46 lines)
...
```
app/orion/orchestrator.py                        (3 changes)
...
### 1. `app/orion/plan_adapter.py` — ExecutionPlanAdapter (new)

Converts `app.orion.planner_schema.ExecutionPlan` → `app.workflow_runtime.models.ExecutionPlanInput`:
...
### 2. `app/orion/orchestrator.py` — Runtime Integration
...
return OrionOrchestrator(..., runtime_bridge=runtime_bridge)
```
...
app/api/routes.py :: get_orchestrator()
  ...
  │
  ▼
OrionOrchestrator.process_query(prompt)
  │
  ...
  │
  ▼
OrionResponse (success, intent, response, telemetry, ...)
```
...
- **No existing API changes**: `OrionOrchestrator.__init__` added `runtime_bridge` as optional. All existing callers (tests) continue working.
...
- `app/orion/executor.py` — No changes needed (fallback path intact)
- `app/orion/planner_schema.py` — No changes needed
- `app/orion/planner_manager.py` — No changes needed
- `app/orion/planner_engine.py` — No changes needed
