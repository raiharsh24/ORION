...
async def sync_mission_progress(manager: MissionManager):
    ...
    for m_id, mission in list(manager._active_missions.items()):
        if mission.status == MissionStatus.RUNNING:
            ...
            event_bus = OrionKernel.get_instance().get_service("event_bus")
            ...
            if progress >= 100.0:
                ...
                if event_bus:
                    ...
                    await event_bus.publish(OrionEvent("MissionCompleted", detailed.model_dump()))
            else:
                if event_bus:
                    detailed = map_to_detailed(mission, manager)
                    await event_bus.publish(OrionEvent("MissionUpdated", detailed.model_dump()))
...
async def start_mission(mission_id: str, manager: MissionManager = Depends(get_mission_manager)) -> StatusToggleResponse:
    ...
    if success:
        ...
        if event_bus:
            from app.events.events import OrionEvent
            detailed = map_to_detailed(mission, manager)
            await event_bus.publish(OrionEvent("MissionStarted", detailed.model_dump()))
...
async def cancel_mission(mission_id: str, manager: MissionManager = Depends(get_mission_manager)) -> StatusToggleResponse:
    ...
    if success:
        ...
        if event_bus:
            from app.events.events import OrionEvent
            detailed = map_to_detailed(mission, manager)
            await event_bus.publish(OrionEvent("MissionCancelled", detailed.model_dump()))
...
async def confirm_mission_action(id: str, request: ActionConfirmRequest) -> Dict[str, Any]:
    automation = OrionKernel.get_instance().get_service("desktop_automation")
    if not automation:
