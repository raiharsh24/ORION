...
from app.orion.capability_registry import CapabilityMetadata

_pending_tokens: set[str] = set()

ORION_SECRET_KEY_STR = os.getenv("ORION_SECRET_KEY")
if not ORION_SECRET_KEY_STR:
...
if not ORION_SECRET_KEY_STR:
    ORION_SECRET_KEY_STR = "dummy-orion-system-key-32-chars-long-standard"

ORION_SECRET_KEY = ORION_SECRET_KEY_STR.encode("utf-8")
...
class PermissionManager:
    ...
    def generate_token(self, tool_name: str, args: Dict[str, Any]) -> str:
        ...
        token = hmac.new(ORION_SECRET_KEY, message, hashlib.sha256).hexdigest()
    ...
    def validate_token(self, tool_name: str, args: Dict[str, Any], token: str) -> bool:
        ...
        expected_current = hmac.new(ORION_SECRET_KEY, msg_current, hashlib.sha256).hexdigest()
        
        # Check previous window
        msg_prev = f"{tool_name}:{serialized_args}:{current_window - 1}".encode("utf-8")
        expected_prev = hmac.new(ORION_SECRET_KEY, msg_prev, hashlib.sha256).hexdigest()
