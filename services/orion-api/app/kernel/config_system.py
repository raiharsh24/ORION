...
class OrionConfigSystem:
    ...
    def __init__(self, config_input: Any = None) -> None:
        if isinstance(config_input, OrionKernelConfig):
            self._config = config_input
        else:
            self._config_class = config_input or OrionKernelConfig
            self._config = self._load_initial_config()

    def _load_initial_config(self) -> OrionKernelConfig:
    ...
    def get_config(self) -> OrionKernelConfig:
        """Returns the current loaded typed configuration instance."""
        return self._config

    def update_config(self, updates: Dict[str, Any]) -> OrionKernelConfig:
        """
