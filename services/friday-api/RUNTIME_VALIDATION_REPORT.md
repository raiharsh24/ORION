# FRIDAY Runtime Validation Report
...
User Request (HTTP /ask or /chat)
  │
  ▼
app/friday/orchestrator.py :: FridayOrchestrator.process_query()
  │
...
|---|-------|------|-------------|
| C1 | **Planner→WorkflowRuntime type mismatch** | `app/friday/planner_schema.py` vs `app/workflow_runtime/models.py` | `PlannerManager.create_plan()` returns `ExecutionPlan`. `RuntimeSchedulerBridge.submit_plan()` expects `ExecutionPlanInput`. They share only `goal` and `steps`. No adapter exists. **The workflow runtime cannot receive plans from the planner.** |
...
| C3 | **FridayOrchestrator bypasses both workflow systems** | `app/friday/orchestrator.py` | `process_query()` uses `ToolExecutor` directly. It does not consult `WorkflowEngine`, `WorkflowRuntimeManager`, or any workflow subsystem. The entire workflow layer is disconnected from the request path. |

### 2.2 High Issues

| # | Issue | File | Description |
|---|-------|------|-------------|
| H1 | **FridayScheduler is a placeholder** | `app/scheduler/scheduler.py` | `start()` and `stop()` are `pass` with TODOs. Cron scheduling in `RuntimeSchedulerBridge.schedule_plan()` will call `agent_scheduler.schedule()` but the `FridayScheduler` itself does nothing. |
| H2 | **ToolEngine event handlers are stubs** | `app/friday/tool_engine.py:287-303` | Four subscribe handlers (`on_plan_validated`, `on_mission_started`, `on_workflow_started`, `on_memory_updated`) only log. They do not execute any tool logic, validate plans, or respond to events. |
| H3 | **Three parallel capability registries** | `app/capabilities/registry.py`, `app/kernel/capability.py`, `app/friday/capability_registry.py` | Three separate registries with different schemas, no synchronization. The kernel's `FridayCapabilityRegistry` is populated during boot with 9 capabilities. `app/capabilities/` is ABC-based with `BaseCapability`. `app/friday/capability_registry.py` has its own Pydantic `CapabilityMetadata`. ToolEngine uses the third one. |
...
| M2 | **Dead code: `app/friday/planner.py` unused imports** | `app/friday/planner.py` | Imports `ToolPlan`, `re` at module level that are never used. These are artifacts from the refactor. |
| M3 | **Dead code: `app/kernel/kernel.py` `register_service()` called from shutdown** | `app/kernel/kernel.py:157-159` | `shutdown()` iterates `list_services()` and calls `unregister_service()` for each. But shutdown is handled by `FridayLifecycleManager.shutdown_all()` which already handles proper lifecycle. The manual loop is redundant. |
...
User Request
  ...
  │     └── app/friday/orchestrator.py
  │           ├── app/friday/intent.py (IntentClassifier)
  │           ├── app/friday/planner.py → PlannerEngine → PlannerManager → LLMRouter
  │           ├── app/friday/tool_executor.py → ToolEngine → ToolResolver → ToolRegistry → app/tools/*
  │           │     └── app/friday/capability_registry.py
  │           │     └── app/friday/permissions.py
  │           │     └── app/friday/sandbox.py
  ...
  │           └── app/friday/prompts.py (PromptManager)
  │
...
FridayServiceContainer
  ...
  ├── "scheduler"                 ─── FridayScheduler (Step 10)
...
```
Step  1: Load Configuration (from FridayKernelConfig)
...
Step 10: FridayScheduler (DI: "scheduler" — PLACEHOLDER)
...
| `app/friday/knowledge_embeddings.py` | 26 | `Placeholder class for future OpenAI embedding...` |
...
| `app/friday/vectordb.py` | `query()`, `upsert()`, `delete()` all `pass` |
| `app/friday/knowledge_embeddings.py` | `generate_embeddings()` is `pass` |
| `app/friday/tool_sandbox.py` | `validate()` is `pass` |
...
3. [C3] Connect Orchestrator → Workflow Runtime
   - In app/friday/orchestrator.py process_query(): after Planner produces
...
```
4. [H1] Implement FridayScheduler
...
6. [H4] Fix fire-and-forget event tasks
   - Add tracked_tasks set to FridayKernel or service base class
...
7. [H3] Consolidate capability registries
   - Merge app/capabilities/ (ABC) and app/friday/capability_registry.py
     (Pydantic) into app/kernel/capability.py (FridayCapabilityRegistry)
