...
BUILTIN_MISSIONS = {
    ...
    "desktop_demo": {
        ...
        "metadata": {
            "category": "automation",
            "actions": [
                {"action": "show_notification", "message": "Starting desktop automation demo", "title": "FRIDAY"},
                {"action": "open_folder", "folder_path": "/home/warlock"},
                {"action": "write_clipboard", "text": "FRIDAY OS Clipboard content"},
...
class MissionManager:
    ...
    async def create_mission(self, request: CreateMissionRequest) -> MissionResponse:
        ...
        if self._event_bus:
            await self._event_bus.publish(
                FridayEvent("MissionCreated", {"mission_id": mission_id, "name": mission.name})
            )
    ...
    async def queue_mission(self, mission_id: str) -> bool:
        ...
        if self._event_bus:
            await self._event_bus.publish(
                FridayEvent("MissionQueued", {"mission_id": mission_id})
            )
    ...
    async def start_mission(self, mission_id: str) -> bool:
        ...
        if mission.type == MissionType.AUTOMATION:
            ...
            desktop_automation = FridayKernel.get_instance().get_service("desktop_automation")
        ...
        if self._event_bus:
            await self._event_bus.publish(
                FridayEvent("MissionStarted", {"mission_id": mission_id})
            )
    ...
    async def pause_mission(self, mission_id: str) -> bool:
        ...
        if self._event_bus:
            await self._event_bus.publish(
                FridayEvent("MissionPaused", {"mission_id": mission_id})
            )
    ...
    async def cancel_mission(self, mission_id: str) -> bool:
        ...
        if self._event_bus:
            await self._event_bus.publish(
                FridayEvent("MissionCancelled", {"mission_id": mission_id})
            )
    ...
    async def complete_mission(self, mission_id: str) -> bool:
        ...
        if self._event_bus:
            await self._event_bus.publish(
                FridayEvent("MissionCompleted", {"mission_id": mission_id})
            )
    ...
    async def fail_mission(self, mission_id: str, error_message: str) -> bool:
        ...
        if self._event_bus:
            await self._event_bus.publish(
                FridayEvent("MissionFailed", {"mission_id": mission_id, "error": error_message})
            )
