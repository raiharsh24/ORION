...
class MemoryRetriever:
    ...
    using recency (exponential time decay), importance score, and keyword overlap.
    """
    ...
    def calculate_score(self, entry: MemoryEntry, query_tokens: Set[str], now: float) -> float:
        """
    ...
    def retrieve(
            ...
            for key, val in user_memory.preferences.items():
                ...
                candidates.append(
                    MemoryEntry(
                        ...
                        metadata={"user_id": user_memory.user_id, "preference_key": key}
                    )
                )
        ...
        # Score and Sort
        scored_entries = [
            (self.calculate_score(entry, query_tokens, now), entry)
