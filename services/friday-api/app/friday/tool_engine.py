...
from app.friday.capability_registry import CapabilityRegistry, CapabilityMetadata
from app.friday.tool_resolver import ToolResolver, ToolValidator
from app.friday.tool_permission import PermissionManager
from app.friday.tool_sandbox import SandboxManager, SandboxViolation
...
class ToolExecuted(FridayEvent):
...
class SandboxViolationEvent(FridayEvent):
...
class ExecutionManager:
    ...
    async def execute_tool(
        ...
        try:
            ...
            self.active_executions -= 1
            self._safe_publish(FridayEvent(topic="ToolCompleted", data={
...
class ToolEngine:
    ...
    # EventBus callbacks
    def on_plan_validated(self, event: FridayEvent) -> None:
    ...
    def on_mission_started(self, event: FridayEvent) -> None:
    ...
    def on_workflow_started(self, event: FridayEvent) -> None:
        data = event.data if hasattr(event, "data") else {}
