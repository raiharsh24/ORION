...
| C2 | `app/friday/planner_manager.py` | `_safe_publish()` used fire-and-forget `loop.create_task()` — created orphaned asyncio tasks that could be abandoned on shutdown | Tasks are now tracked in `self._pending_tasks` set with `add_done_callback(self._pending_tasks.discard)`; new `async shutdown()` method cancels and drains pending tasks |
