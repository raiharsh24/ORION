...
from app.kernel import (
    FridayKernel, FridayServiceContainer, FridayKernelConfig, KernelState,
...
async def test_kernel_singleton():
    ...
    k1 = FridayKernel.get_instance()
    k2 = FridayKernel.get_instance()
    assert k1 is k2
    
    FridayKernel.reset_instance()
    k3 = FridayKernel.get_instance()
    assert k3 is not k1

@pytest.mark.anyio
async def test_service_container():
    container = FridayServiceContainer()
...
async def test_kernel_config():
    config = FridayKernelConfig.load_defaults()
...
async def test_kernel_boot_shutdown_restart():
    ...
    boot_events = []
    def on_event(event: FridayEvent):
