...
async def test_manager_events_interception():
    ...
    await event_bus.publish(FridayEvent("ConversationCompleted", {"session_id": "sess_xyz"}))
    ...
    await event_bus.publish(FridayEvent("ToolCompleted", {"tool_name": "filesystem", "output": "file list details", "session_id": "sess_xyz"}))
    await anyio.sleep(0.1)
