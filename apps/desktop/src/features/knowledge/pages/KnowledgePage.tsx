      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="font-mono text-xs text-zinc-500 tracking-wider mt-1 uppercase">
            Workspace scanner, semantic code indexing, and local vector retrieval
          </p>
        </div>
        <Button 
          variant="outline" 
          size="sm" 
          onClick={fetchWorkspaceProjects}
          className="h-8 font-mono text-[10px] tracking-wider"
          disabled={indexingInProgress}
        >
          <RefreshCw className={`w-3.5 h-3.5 mr-1.5 ${indexingInProgress ? 'animate-spin' : ''}`} />
          SCAN DIRECTORY
        </Button>
      </div>
