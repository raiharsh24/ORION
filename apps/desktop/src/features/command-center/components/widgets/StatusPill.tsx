...
export const StatusPill: React.FC<{ status: AgentStatus; className?: string }> = ({ status, className = '' }) => {
  ...
  return (
    <span className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full border ${s.ring} bg-black/30 ${className}`}>
      ...
      <span className={`text-[8px] font-mono font-bold uppercase tracking-[0.15em] ${s.text}`}>{status}</span>
    </span>
  );
};
